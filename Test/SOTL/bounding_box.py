...
class WaitTime():
	...
	def start_timer(self):
		if self.start_time == -1:
			...
			# print(f"Id: {self.id} timer started")
		else:
			self.total_wait_time = max(self.total_wait_time, time.time()-self.start_time)
	def stop_timer(self):
		if self.start_time != -1:
			self.total_wait_time += time.time()-self.start_time
			self.start_time = -1
			# print(f"Id: {self.id} timer ended")
			# print(f"Id: {self.id} wait time: {self.total_wait_time}")
...
class TravelTime():
	...
	def stop_timer(self):
		...
		# print(f"Id: {self.id} timer ended")
		# print(f"Id: {self.id} travel time: {self.travel_time}")
	@classmethod
	def avg_travel_time(cls):
		# print(f"len {TravelTime.destination_reached} TTT {TravelTime.total_travel_time}")
