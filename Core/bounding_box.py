...
all_vehicles_wait_time = {} # Dictionary to store wait times for all vehicles
# Class to track wait time for a specific vehicle
class WaitTime():
	def __init__(self, id):
		self.start_time = -1 # Initialize start time to -1 (not started)
		self.total_wait_time = 0 # Initialize total wait time to 0
		self.id = id # Store the vehicle ID
	def start_timer(self):
		if self.start_time == -1:
			self.start_time = time.time() # Start the timer
		else:
			# Accumulate the total wait time if already started
			self.total_wait_time = max(self.total_wait_time, time.time()-self.start_time)
	def stop_timer(self):
		if self.start_time != -1:
			self.total_wait_time += time.time()-self.start_time # Accumulate total wait time
			self.start_time = -1 # Reset start time to -1 (not started)

	def get_wait_time(self):
		return self.total_wait_time   # Return the total accumulated wait time

	
all_vehicles_travel_time = {} # Dictionary to store travel times for all vehicles
# Class to track travel time for a specific vehicle
class TravelTime():
	total_travel_time = 0
...
class LaneDetection:
	...
	def return_lane_detections(world, camera, K, world_2_camera, lane_coordinates):
		...
		for npc in world.get_actors().filter('*vehicle*'):
			...
			#we are restricting our search region between 10 and 60 metric from the traffic signal.
			if 10 < dist < 60:
				...
				# print(f"speed of vehicle: {forward_speed}")

				#dot checks if the vehicle is front of the camera and not behind the camera. Now the vehicles infront of the camera can be either incoming traffic or outgoing.
				#dot_ref constraints the search region to only incoming traffic and not outgoing traffic.
				#point_left helps to get the left lane detections in front of the camera.
				...
				elif dot < -0.78 and dot_ref > 0.85 and not point_right:
					verts = [v for v in bb.get_world_vertices(npc.get_transform())]
					x_max, x_min, y_max, y_min = -10000, 10000, -10000, 10000
